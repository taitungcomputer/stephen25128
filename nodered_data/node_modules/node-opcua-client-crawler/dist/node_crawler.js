...
class NodeCrawler extends node_crawler_base_1.NodeCrawlerBase {
    ...
    read(nodeId, callback) {
        ...
        this.crawl(nodeId, userData, (err) => {
            ...
            if (Object.prototype.hasOwnProperty.call(this._objectCache, key)) {
                ...
                (0, node_opcua_client_1.assert)(cacheNode.browseName.name !== "pending");
    ...
    simplify_object(objMap, object, finalCallback) {
        (0, node_opcua_client_1.assert)(typeof finalCallback === "function");
        const queue = async_1.default.queue((task, innerCallback) => {
            setImmediate(() => {
    ...
    _add_for_reconstruction(queue, objMap, object, extraFunc) {
        ...
        (0, node_opcua_client_1.assert)(typeof extraFunc === "function");
        (0, node_opcua_client_1.assert)(typeof object.nodeId.toString() === "string");
        const task = {
            data: object,
            func: (data, callback) => {
    ...
    _reconstruct_manageable_object(queue, objMap, object, callback) {
        (0, node_opcua_client_1.assert)(typeof callback === "function");
        (0, node_opcua_client_1.assert)(object);
        (0, node_opcua_client_1.assert)(object.nodeId);
        ...
        object.references.map((ref) => {
            (0, node_opcua_client_1.assert)(ref);
            const refIndex = ref.referenceTypeId.toString();
            const referenceType = this._objectCache[refIndex];
            /* istanbul ignore else */
            if (!referenceType) {
                warningLog(chalk_1.default.red("Unknown reference type " + refIndex));
                // debugLog(util.inspect(object, { colors: true, depth: 10 }));
            }
            const reference = this._objectCache[ref.nodeId.toString()];
            /* istanbul ignore else */
            if (!reference) {
                warningLog(ref.nodeId.toString(), "bn=", ref.browseName.toString(), "class =", node_opcua_client_1.NodeClass[ref.nodeClass], ref.typeDefinition.toString());
            ...
            if (referenceType) {
                const refName = (0, node_opcua_client_1.lowerFirstLetter)(referenceType?.browseName?.name || "");
