...
const debugLog = (0, node_opcua_client_1.make_debugLog)(__filename);
const doDebug = (0, node_opcua_client_1.checkDebugFlag)(__filename);
const doDebug1 = doDebug && false;
const warningLog = (0, node_opcua_client_1.make_warningLog)(__filename);
console.log("+-------------------------------------------------------------------------------------+");
console.log("| Warning:                                                                            |");
...
const resultMask = (0, node_opcua_client_1.makeResultMask)("ReferenceType | IsForward | BrowseName | DisplayName | NodeClass | TypeDefinition");
function zip(arrA, arrB) {
    return arrA.map((value, idx) => [value, arrB[idx]]);
}
function make_node_attribute_key(nodeId, attributeId) {
    return nodeId.toString() + "_" + node_opcua_client_1.AttributeIds[attributeId];
}
...
function _fetch_elements(arr, maxNode) {
    (0, node_opcua_client_1.assert)(Array.isArray(arr));
    (0, node_opcua_client_1.assert)(arr.length > 0);
    const highLimit = maxNode <= 0 ? arr.length : maxNode;
    const tmp = arr.splice(0, highLimit);
    (0, node_opcua_client_1.assert)(tmp.length > 0);
    return tmp;
}
const referencesNodeId = (0, node_opcua_client_1.resolveNodeId)("References");
// const hierarchicalReferencesId = resolveNodeId("HierarchicalReferences");
const hasTypeDefinitionNodeId = (0, node_opcua_client_1.resolveNodeId)("HasTypeDefinition");
function _setExtraReference(task, callback) {
    const param = task.param;
    (0, node_opcua_client_1.assert)(param.userData.setExtraReference);
...
function getReferenceTypeId(referenceType) {
    ...
    if (referenceType.toString() === "i=45" || referenceType === "HasSubtype") {
        return node_opcua_client_1.NodeId.resolveNodeId("i=45");
    }
    else if (referenceType.toString() === "i=35" || referenceType === "Organizes") {
        return node_opcua_client_1.NodeId.resolveNodeId("i=35");
    }
    else if (referenceType.toString() === "i=47" || referenceType === "HasComponent") {
        return node_opcua_client_1.NodeId.resolveNodeId("i=47");
    }
    else if (referenceType.toString() === "i=46" || referenceType === "HasProperty") {
        return node_opcua_client_1.NodeId.resolveNodeId("i=46");
    }
    else if (referenceType.toString() === node_opcua_client_1.NodeId.resolveNodeId("HasEncoding").toString() || referenceType === "HasEncoding") {
        return node_opcua_client_1.NodeId.resolveNodeId("HasEncoding");
    }
    else if (referenceType.toString() === node_opcua_client_1.NodeId.resolveNodeId("HasDescription").toString() ||
        referenceType === "HasDescription") {
        return node_opcua_client_1.NodeId.resolveNodeId("HasDescription");
    }
    else if (referenceType.toString() === "i=31" || referenceType === "References") {
        return node_opcua_client_1.NodeId.resolveNodeId("i=31");
    }
    else {
...
class NodeCrawlerBase extends events_1.EventEmitter {
    static follow(crawler, cacheNode, userData, referenceType, browseDirection) {
        ...
        for (const reference of cacheNode.references) {
            if (browseDirection === node_opcua_client_1.BrowseDirection.Forward && !reference.isForward) {
                continue;
            }
            if (browseDirection === node_opcua_client_1.BrowseDirection.Inverse && reference.isForward) {
                continue;
            }
            if (!referenceTypeNodeId) {
                ...
                if (node_opcua_client_1.NodeId.sameNodeId(referenceTypeNodeId, reference.referenceTypeId)) {
                    crawler.followReference(cacheNode, reference, userData);
                }
            }
        }
    ...
    constructor(session) {
        ...
        this.taskQueue = async_1.default.queue((task, callback) => {
    ...
    dispose() {
        (0, node_opcua_client_1.assert)(this.pendingReadTasks.length === 0);
        (0, node_opcua_client_1.assert)(this.pendingBrowseTasks.length === 0);
        this.pendingReadTasks.length = 0;
        this.pendingBrowseTasks.length = 0;
        (0, node_opcua_client_1.assert)(this.taskQueue.length() === 0);
    ...
    crawl(nodeId, userData, ...args) {
        ...
        (0, node_opcua_client_1.assert)(typeof endCallback === "function", "expecting callback");
        nodeId = (0, node_opcua_client_1.resolveNodeId)(nodeId);
        (0, node_opcua_client_1.assert)(typeof endCallback === "function");
    ...
    _inner_crawl(nodeId, userData, endCallback) {
        (0, node_opcua_client_1.assert)(userData !== null && typeof userData === "object");
        (0, node_opcua_client_1.assert)(typeof endCallback === "function");
        let hasEnded = false;
        ...
        async_1.default.parallel({
            task1: (callback) => {
                this._defer_readNode(cacheNode.nodeId, node_opcua_client_1.AttributeIds.BrowseName, (err, value) => {
                    /* istanbul ignore else */
                    if (err) {
                        return callback(err);
                    }
                    if (!(value instanceof node_opcua_client_1.QualifiedName)) {
                        warningLog(" node ", cacheNode.nodeId.toString(), " has a invalid browseName", value);
                        cacheNode.browseName = (0, node_opcua_client_1.coerceQualifiedName)("<INVALID BROWSE NAME>");
                    }
                    else {
            ...
            task2: (callback) => {
                this._defer_readNode(cacheNode.nodeId, node_opcua_client_1.AttributeIds.NodeClass, (err, value) => {
                    /* istanbul ignore else */
                    if (err) {
                        return callback(err);
                    }
            ...
            task3: (callback) => {
                this._defer_readNode(cacheNode.nodeId, node_opcua_client_1.AttributeIds.DisplayName, (err, value) => {
                    /* istanbul ignore else */
                    if (err) {
                        return callback(err);
                    }
                    if (!(value instanceof node_opcua_client_1.LocalizedText)) {
                        warningLog(" node ", cacheNode.nodeId.toString(), " has a invalid displayName", value);
                        cacheNode.displayName = (0, node_opcua_client_1.coerceLocalizedText)("<INVALID LOCALIZED NAME>");
                    }
                    else {
    ...
    _add_crawl_task(cacheNode, userData) {
        (0, node_opcua_client_1.assert)(userData);
        (0, node_opcua_client_1.assert)(this !== null && typeof this === "object");
    ...
    followReference(parentNode, reference, userData) {
        (0, node_opcua_client_1.assert)(reference instanceof node_opcua_client_1.ReferenceDescription);
        ...
        if (!referenceTypeIdCacheNode) {
            ...
            referenceTypeIdCacheNode.nodeClass = node_opcua_client_1.NodeClass.ReferenceType;
            this._add_crawl_task(referenceTypeIdCacheNode, userData);
        }
        ...
        if (!childCacheNode) {
            ...
            (0, node_opcua_client_1.assert)(childCacheNode.parent === parentNode);
            (0, node_opcua_client_1.assert)(childCacheNode.referenceToParent === reference);
    ...
    _resolve_deferred_readNode(callback) {
        ...
        this.session
            ...
            for (const [readTask, dataValue] of zip(selectedPendingReadTasks, dataValues)) {
                (0, node_opcua_client_1.assert)(Object.prototype.hasOwnProperty.call(dataValue, "statusCode"));
                if (dataValue.statusCode.equals(node_opcua_client_1.StatusCodes.Good)) {
                    /* istanbul ignore else */
    ...
    _resolve_deferred_browseNode(callback) {
        ...
        const nodesToBrowse = objectsToBrowse.map((e) => {
            (0, node_opcua_client_1.assert)(Object.prototype.hasOwnProperty.call(e, "referenceTypeId"));
            return new node_opcua_client_1.BrowseDescription({
                browseDirection: node_opcua_client_1.BrowseDirection.Forward,
        ...
        (0, node_opcua_client_1.browseAll)(this.session, nodesToBrowse)
            .then((browseResults) => {
            (0, node_opcua_client_1.assert)(browseResults.length === nodesToBrowse.length);
    ...
    _unshift_task(name, task) {
        (0, node_opcua_client_1.assert)(typeof task.func === "function");
        (0, node_opcua_client_1.assert)(task.func.length === 2);
    ...
    _crawl_task(task, callback) {
        ...
        const browseNodeAction = (err, cacheNode1) => {
            ...
            for (const reference of cacheNode1.references) {
                ...
                if (!this.has_cache_NodeAttribute(reference.nodeId, node_opcua_client_1.AttributeIds.BrowseName)) {
                    this.set_cache_NodeAttribute(reference.nodeId, node_opcua_client_1.AttributeIds.BrowseName, reference.browseName);
                }
                if (!this.has_cache_NodeAttribute(reference.nodeId, node_opcua_client_1.AttributeIds.DisplayName)) {
                    this.set_cache_NodeAttribute(reference.nodeId, node_opcua_client_1.AttributeIds.DisplayName, reference.displayName);
                }
                if (!this.has_cache_NodeAttribute(reference.nodeId, node_opcua_client_1.AttributeIds.NodeClass)) {
                    this.set_cache_NodeAttribute(reference.nodeId, node_opcua_client_1.AttributeIds.NodeClass, reference.nodeClass);
                }
            }
    ...
    _initialize_referenceTypeId() {
        const appendPrepopulatedReference = (browseName) => {
            const nodeId = (0, node_opcua_client_1.makeNodeId)(node_opcua_client_1.ReferenceTypeIds[browseName], 0);
            (0, node_opcua_client_1.assert)(nodeId);
            const cacheNode = this._createCacheNode(nodeId);
            cacheNode.browseName = new node_opcua_client_1.QualifiedName({ name: browseName });
            cacheNode.nodeClass = node_opcua_client_1.NodeClass.ReferenceType;
    ...
    _readOperationalLimits(callback) {
        const n1 = (0, node_opcua_client_1.makeNodeId)(node_opcua_client_1.VariableIds.Server_ServerCapabilities_OperationLimits_MaxNodesPerRead);
        const n2 = (0, node_opcua_client_1.makeNodeId)(node_opcua_client_1.VariableIds.Server_ServerCapabilities_OperationLimits_MaxNodesPerBrowse);
        const nodesToRead = [
            { nodeId: n1, attributeId: node_opcua_client_1.AttributeIds.Value },
            { nodeId: n2, attributeId: node_opcua_client_1.AttributeIds.Value }
        ];
        this.transactionCounter++;
        this.session
            .read(nodesToRead)
        ...
        if (this.has_cache_NodeAttribute(nodeId, attributeId)) {
            ...
            this.pendingReadTasks.push({
                action: (value, dataValue) => {
                    if (attributeId === node_opcua_client_1.AttributeIds.Value) {
                        this.set_cache_NodeAttribute(nodeId, attributeId, dataValue);
                        callback(null, dataValue);
                        return;
                    }
                    if (attributeId === node_opcua_client_1.AttributeIds.ArrayDimensions) {
                    ...
                    if (dataValue.statusCode.isGood()) {
                        ...
                        callback(new Error("Error " +
                            ...
                            nodeId.toString() +
                            " attributeIds " +
                            node_opcua_client_1.AttributeIds[attributeId]));
                    }
                },
                nodeToRead: {
                    attributeId,
    ...
    _getCacheNode(nodeId) {
        const key = (0, node_opcua_client_1.resolveNodeId)(nodeId).toString();
    ...
    _createCacheNode(nodeId, parentNode, referenceToParent) {
        ...
        const nodeClass = (referenceToParent ? referenceToParent.nodeClass : node_opcua_client_1.NodeClass.Unspecified);
        switch (nodeClass) {
            case node_opcua_client_1.NodeClass.Method:
                cacheNode = new cache_node_1.CacheNode(nodeId);
                cacheNode.nodeClass = node_opcua_client_1.NodeClass.Method;
                break;
            case node_opcua_client_1.NodeClass.Object:
                cacheNode = new cache_node_1.CacheNode(nodeId);
                cacheNode.nodeClass = node_opcua_client_1.NodeClass.Object;
                break;
            case node_opcua_client_1.NodeClass.ObjectType:
                cacheNode = new cache_node_1.CacheNode(nodeId);
                cacheNode.nodeClass = node_opcua_client_1.NodeClass.ObjectType;
                break;
            case node_opcua_client_1.NodeClass.Variable:
                cacheNode = new cache_node_1.CacheNodeVariable(nodeId);
                break;
            case node_opcua_client_1.NodeClass.VariableType:
        ...
        (0, node_opcua_client_1.assert)(!Object.prototype.hasOwnProperty.call(this._objectCache, key));
    ...
    _process_single_browseResult(_objectToBrowse, browseResult) {
        ...
        (0, node_opcua_client_1.assert)(this._visitedNode.has(cacheNode.nodeId.toString()));
    ...
    _process_single_browseResult2(_objectToBrowse) {
        ...
        const tmp = cacheNode.references.filter((x) => (0, node_opcua_client_1.sameNodeId)(x.referenceTypeId, hasTypeDefinitionNodeId));
        if (tmp.length) {
            // istanbul ignore next
            if (tmp.length !== 1) {
        ...
        async_1.default.parallel({
            task1_read_browseName: (callback) => {
                ...
                this._defer_readNode(cacheNode.nodeId, node_opcua_client_1.AttributeIds.BrowseName, (err, browseName) => {
            ...
            task3_read_description: (callback) => {
                this._defer_readNode(cacheNode.nodeId, node_opcua_client_1.AttributeIds.Description, (err, value) => {
                    if (err) {
                        // description may not be defined and this is OK !
                        return callback();
                    }
                    cacheNode.description = (0, node_opcua_client_1.coerceLocalizedText)(value);
                    callback();
                });
            },
            task4_variable_dataType: (callback) => {
                // only if nodeClass is Variable || VariableType
                if (cacheNode.nodeClass !== node_opcua_client_1.NodeClass.Variable && cacheNode.nodeClass !== node_opcua_client_1.NodeClass.VariableType) {
                    return callback();
                }
                const cache = cacheNode;
                // read dataType and DataType if node is a variable
                this._defer_readNode(cacheNode.nodeId, node_opcua_client_1.AttributeIds.DataType, (err, dataType) => {
                    if (!(dataType instanceof node_opcua_client_1.NodeId)) {
                        return callback();
                    }
            ...
            task5_variable_dataValue: (callback) => {
                ...
                const cache = cacheNode;
                this._defer_readNode(cacheNode.nodeId, node_opcua_client_1.AttributeIds.Value, (err, value) => {
                    if (!err) {
                        (0, node_opcua_client_1.assert)(value instanceof node_opcua_client_1.DataValue);
            ...
            task6a_variable_arrayDimension: (callback) => {
                ...
                const cache = cacheNode;
                this._defer_readNode(cacheNode.nodeId, node_opcua_client_1.AttributeIds.ArrayDimensions, (err, value) => {
                    if (!err) {
            ...
            task6b_variable_valueRank: (callback) => {
                ...
                const cache = cacheNode;
                this._defer_readNode(cacheNode.nodeId, node_opcua_client_1.AttributeIds.ValueRank, (err, value) => {
                    if (!err) {
            ...
            task7_variable_minimumSamplingInterval: (callback) => {
                if (cacheNode.nodeClass !== node_opcua_client_1.NodeClass.Variable) {
                    return callback();
                }
                const cache = cacheNode;
                this._defer_readNode(cacheNode.nodeId, node_opcua_client_1.AttributeIds.MinimumSamplingInterval, (err, value) => {
            ...
            task8_variable_accessLevel: (callback) => {
                ...
                const cache = cacheNode;
                this._defer_readNode(cacheNode.nodeId, node_opcua_client_1.AttributeIds.AccessLevel, (err, value) => {
                    if (err) {
                        return callback(err);
                    }
            ...
            task9_variable_userAccessLevel: (callback) => {
                ...
                const cache = cacheNode;
                this._defer_readNode(cacheNode.nodeId, node_opcua_client_1.AttributeIds.UserAccessLevel, (err, value) => {
                    if (err) {
                        return callback(err);
                    }
            ...
            taskA_referenceType_inverseName: (callback) => {
                if (cacheNode.nodeClass !== node_opcua_client_1.NodeClass.ReferenceType) {
                    return callback();
                }
                const cache = cacheNode;
                this._defer_readNode(cacheNode.nodeId, node_opcua_client_1.AttributeIds.InverseName, (err, value) => {
                    if (err) {
                        return callback(err);
                    }
            ...
            taskB_isAbstract: (callback) => {
                ...
                const cache = cacheNode;
                this._defer_readNode(cacheNode.nodeId, node_opcua_client_1.AttributeIds.IsAbstract, (err, value) => {
                    if (err) {
                        return callback(err);
                    }
            ...
            taskC_dataTypeDefinition: (callback) => {
                if (cacheNode.nodeClass !== node_opcua_client_1.NodeClass.DataType) {
                    return callback();
                }
                // dataTypeDefinition is new in 1.04
                const cache = cacheNode;
                this._defer_readNode(cacheNode.nodeId, node_opcua_client_1.AttributeIds.DataTypeDefinition, (err, value) => {
                    if (err) {
    ...
    _process_browse_response_task(task, callback) {
        ...
        for (const [objectToBrowse, browseResult] of zip(objectsToBrowse, browseResults)) {
            (0, node_opcua_client_1.assert)(browseResult instanceof node_opcua_client_1.BrowseResult);
